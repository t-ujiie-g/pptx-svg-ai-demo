...
PPTX編集の連携:
- ユーザーのメッセージに「【編集中PPTX — artifact_id: ...】」というコンテキストが含まれている場合、\
これはフロントエンドで表示中のPPTXの構造情報です。
- この場合は artifact_id を {pptx_agent.name} に伝えて、既存PPTX編集フローを使うよう委譲してください。
- テキスト修正、色変更、レイアウト調整など「既存スライドの修正」はすべて編集フローを使います。
- 新しいプレゼンテーションをゼロから作る場合のみ新規作成フローを使います。
